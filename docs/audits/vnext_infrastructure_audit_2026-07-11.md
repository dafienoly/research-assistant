...
## 2026-07-12 第三轮复核：周维护、增量写入与门禁语义

本轮从实际运行中的周维护进程、当前 canonical 审计产物和 CI 测试结果复核，发现以下问题并已修复：

| 发现 | 证据 | 修复 | 验收 |
|---|---|---|---|
| 周维护仍调用不存在的 `concept`/`industry` API，失败后重复重试并拖住财务逐股拉取 | `datahub_cron.log` 连续出现“请指定正确的接口名”，周维护进程运行超过 1 小时 | `incremental_concept_industry` 统一委托 `pull_concept_industry_mx`，使用 `ths_index`/`index_classify`，保留来源、质量和错误；财务接口连续空结果/失败达到阈值即熔断，已有文件不覆盖 | Data pipeline/scheduling/architecture 61 项通过；后续周任务不再走旧 API |
| 每日增量在已有历史上反复读取、去重并重写数千 CSV | `incremental_daily`、估值和资金流原先每个标的都调用全量合并 | 新增受控 `incremental_state.json`：确认前一交易日完整后采用追加写入；状态缺失/同日重跑仍走安全合并；历史文件缺失或 schema 改变拒绝静默重建 | 追加、缺失历史保护和 staging 测试覆盖；返回 `write_mode=append_only/merge_once` |
| Readiness CLI 与 cron 使用不同凭据环境，且 Shadow 缺失时 warning 仍可显示 READY | 直接运行 readiness 曾显示 Telegram 未配置，Shadow 为 `warning` | CLI 只解析 allow-list 的 `~/.config/hermes/runtime.env`，不执行 shell；Shadow 少于 5 个 canonical 交易日改为 blocker | Readiness 13 门禁：12 pass、1 blocker（Shadow），不再误报 READY；通知凭据不输出 |
| K 线验收测试读取废弃空目录，无法反映 DataHub 真相，并将 UTF-8 BOM 误判为 schema 错误 | `data/market/daily_kline` 为空，真实数据在 `data/normalized/market` | 测试通过 `daily_kline_root` 和 `stock_basic` 过滤 canonical U0，兼容 `trade_date/vol` 与 BOM，排除非 canonical 测试残留 | K 线数据测试 7/7 通过；核心 coverage 5,530/5,530、freshness/integrity OK |
| 测试把 Alpha 注册写入 D 盘真实 Registry，造成目录无限增长并拖慢全量测试 | `test_core_framework`、`test_alpha_factory` 使用生产常量；一次运行新增数百目录 | 两个测试套件统一重定向到 pytest 临时 Registry；生产数据不清理、不回滚 | Registry/core/concurrency 25 项通过，后续测试不再污染 D 盘 |

本轮验证：后端 DataHub/决策/QMT/VNext/Shadow 专项 326 项通过；数据管线与 readiness 新增回归 165 项通过；前端 lint、Vitest 28 项和 Vite build 全部通过；pre-push 完整审计 0 fail/0 warn。当前真实阻断仍保持诚实：连续 Shadow、浏览器 console/点击/截图证据和 MiniQMT 实盘验收缺失；MiniQMT 实测已按用户授权跳过，实盘开关继续关闭。辅助 `preopen_events.csv` 为空只产生非阻塞 watch-only 缺口，不得被当作 BUY 依据。

## 2026-07-12 第四轮复核：架构审计扫描边界与回归时长

上一轮全量 pytest 在 `test_architecture_audit.py::test_cli_inventory` 附近长时间无输出。复核确认根因不是业务死循环，而是审计器对 `/mnt/d/HermesReports` 进行多次无界 `Path.rglob`：同一批历史产物被 HTML/CSV/JSON/MD/LOG 五次遍历，日志又被单独递归读取。该行为会放大挂载盘元数据延迟，并可能把测试总时长推到十分钟以上。

现已完成以下工程修复：

- `architecture_audit.py` 统一使用受控 `os.scandir` 遍历，跳过 `.git`、虚拟环境、构建产物、缓存及原始/规范化高频数据目录；源码、产物和日志扫描均设置深度/文件数上限，产物行显式写入 `scan_limit` 与 `truncated`，不会把样本当成全量事实。
- 多后缀产物统计合并为一次遍历，日志只读取有限审计日志样本；完整行情/数据完整性继续由 DataHub audit 负责，架构审计不再承担第二套数据扫描职责。
- 架构审计测试改为 module-scope fixture，一次扫描覆盖 10 个文件断言，避免测试自身重复触发 D 盘遍历。

验收：旧架构审计测试 `10 passed in 6.44s`，K 线 canonical 数据测试 `7 passed in 1.31s`；生产审计模块、DataHub 管线和 readiness 模块 Ruff 检查全部通过。随后该旧架构扫描器已整体退役，新的 release-only source audit 永久禁用 artifact/data/temp 扫描，不提供扩大范围的兼容开关。

## 2026-07-12 第五轮复核：鉴权测试与真实失败语义

一次完整后端回归（修复前）收集 3,001 项，结果为 `2,971 passed / 13 skipped / 17 failed`。失败并非业务数据被删除：

- 11 项数据状态、运维和报告 API 测试没有携带当前已启用的 `HERMES_UI_TOKEN`，导致真实鉴权返回 401 后产生连锁断言失败；测试现从进程环境或项目私有 `.env` 读取 token，仅放入 TestClient 请求头，不打印或写入仓库。
- 通知运行环境测试原先错误假设 UI token 一定不存在；现验证 allow-list loader 不会改变既有 UI token，且仍不会加载 `HERMES_UI_TOKEN`。
- 资金流因子测试使用 `returns` 作为同日输入，触发正确的未来函数阻断；测试改为使用不泄露未来的价量窗口与 `net_main_force` 组合。
- VNext lane 测试原来写死某一次 artifact 的缺口字段和“必须在容差内”；现按 reconciliation 的真实容差计算验证 `within_tolerance`，按事件真值 manifest 的实际状态验证缺口，仍强制 `BACKTEST_ONLY` 不得晋级。

修复后，原失败集合及相邻契约共 `22 passed`；架构/数据/readiness 196 项、决策/QMT/VNext/Shadow/后端 352 项和 VNext lane 4 项均通过。回归过程中未删除或覆盖项目数据；全量测试生成的运行笔记、审计快照和组合产物仍原样保留，未纳入代码提交。

## 2026-07-12 第六轮复核：退役旧审计、改为大版本源码审计

旧审计系统的主要问题已确认：pre-push 每次推送触发 full profile，full profile 又可能递归运行受影响 pytest；架构审计对 D 盘报告树执行多轮 `rglob`；CI 还会每个 PR/Push 运行 Semgrep 与 GitNexus。这些流程把数据/临时 I/O 混入代码门禁，导致重复扫描、长时间占用和错误归因。

现已完成切换：

- `factor_lab.audit.source_audit` 成为唯一源码审计内核。它只读取 Git 变更中的 `.py/.js/.ts/.sh/.yml/.yaml/.toml`，明确排除 `data`、`artifacts`、`research_outputs`、`tmp/temp`、缓存、虚拟环境、构建目录和 `node_modules`；文件大小与数量有硬上限。
- 普通 `audit:code`、`leader:anti-cheat-audit`、`architecture:audit` 和 API trigger 在未提供 `major_version` 时只返回结构化 `SKIPPED`，不读仓库文件、不跑 pytest、Semgrep、GitNexus，也不写审计运行记录。
- `--major-version 2.0.0` 是唯一显式开关；`commands/scripts/major_code_audit.sh` 提供发布前命令。运行结果只写 `~/.hermes/state/research-assistant/code-audits`，架构报告只写用户状态目录或显式 `/tmp`，不写数据盘。
- pre-commit/pre-push 改为空钩子；CI 删除每次 PR/Push 的代码审计和 GitNexus 步骤，增加手工 workflow dispatch 的 release-only source audit 输入。
- CI 中残留的手工 Semgrep blocking step 也已移除；它属于旧 Gate 链，不能绕过 source-only 审计边界。发布前审计只执行同一套显式 major-version 源码内核。
- 旧 Gate/LLM/pytest 编排模块保留为历史兼容代码但已无生产调用路径；数据健康、DataHub integrity、执行审计账本仍属于业务安全闭环，不与源码审计混用。

验收：普通 CLI 与 legacy alias 均在亚秒级返回 `SKIPPED`；显式 `--major-version 2.0.0 --scope paths` 对单个源码文件通过，报告明确标记 `data_scan=false/temp_scan=false/pytest=false/semgrep=false/gitnexus=false`；源码审计与架构审计契约测试 `19 passed`。未删除任何数据文件。

## 2026-07-12 第七轮复核：辅助门禁与盘后复盘统一 DataHub

分钟 Decision Loop 的辅助数据门禁原先会自行 glob `fund_flow`、fundamentals 和事件目录，并用 CSV 文件 mtime 推断新鲜度。这会让业务层重新实现一套数据发现逻辑，也会把“文件存在”误报为“可执行数据”。现已收口到 `data/audit/manifests/factor_input_projection.json`：news、fund-flow、fundamentals 只接受 manifest 的状态、观测时间、来源和证据；manifest 缺失、无效、过期或 `EMPTY/BLOCKED/MISSING/INVALID` 均 fail-closed。来源冲突继续由 DataHub conflict ledger 汇总，交给统一 data gate 降为 `WATCH_ONLY`；核心行情/持仓/日历异常仍为 `BLOCKED`。健康快照使用临时文件、`fsync` 和 `os.replace` 原子发布，避免并发分钟任务读取半截 JSON。

盘后 ReviewRecord 原先只使用当日 cycle 价格路径，1/5/20 日收益和相对基准常常为空或只反映模拟路径。现复盘通过 `datahub_access.daily_kline_path` 读取 canonical 日线，检测到多个不同哈希时直接拒绝；不做 provider fallback、目录外搜索或合成收益。个股和动态匹配的行业/ETF 基准均使用同一 canonical 路径，缺失时保留 null 和缺失原因，不把缺数据当零收益。半导体设备 ETF 的高开回落场景由历史回放认证测试覆盖，实盘执行仍保持关闭。

本轮验收：`test_decision_loop.py` 37 项通过，DataHealth/Review 相关 Ruff 检查通过；真实运行的三项辅助门禁均从 `canonical_datahub_projection` 读取，当前冲突数为 0。仍未宣称完成的外部证据为连续 Shadow 运行、MiniQMT 实测（用户无权限）和浏览器 console/点击/截图验收。

同轮补查盘中低频兼容入口时发现，成交额监测在实时快照完全缺失或 `amount` 全部不可解析时可能把零值继续当成有效行情。现改为 `MISSING` 并保留明确原因/坏行计数；只有通过 DataHub 实时快照校验且历史 turnover manifest/hash 新鲜时才返回 `OK`。同时清理该入口及其测试的遗留 Ruff 错误，避免工程门禁被无关 warning 淹没。`test_intraday_monitor.py` 34 项通过，模块与测试 Ruff 全部通过。

VNext PassList 烟测得到 0 主推荐、0 备选，符合不凑数策略；原空结果文案只统计 `BLOCKED`，无法解释九个候选为何处于 `WATCH_ONLY`。现同时输出执行/辅助门禁观察数量和去重后的阻断原因，例如 `VNext data health=PARTIAL`、`mandatory execution checks are missing or the asset is restricted`，并明确保持现金。该状态继续写入分钟 watchlist，但不会生成 BUY 计划。
...
同轮补齐组合动态基准：组合接口现在按实际暴露调用已验证的个股行业 ETF/指数或 ETF 跟踪指数/同类 ETF 映射，并可单独提供 `tradable_benchmarks`；没有可靠映射的暴露进入 `unmapped` 原因，不再把原始持仓符号直接当作基准。单标的和组合映射专项回归通过，缺失映射保持 `components=null`。

参数晋级链同时增加 candidate 级 operation lock 和幂等 production-history key；同一 candidate 的并发周度批准最多产生一个晋级版本，后续重复请求返回已晋级状态，不会重复写生产参数。

受控执行边界也增加 order-level claim lock：同一 order/event 的并发提交在 broker 调用前串行化，第二个 worker 返回 `execution_order_overlap`，已提交订单或事件再次进入时返回幂等阻断；不同订单仍可并行，QMT 实盘开关和授权门禁保持原有 fail-closed 约束。

QMT 只读对账同样改为 `reconciliation/sync` 单飞锁；账户/持仓预览和人工确认不会互相覆盖，连续失败计数使用 `update_json` 原子递增，达到阈值仍撤销当日授权。无 MiniQMT 权限时接口仍只返回明确不可用，不生成空持仓。
