...
VNext 的安全门禁和决策闭环已经成形，VNext、Decision Loop、股票池、动态基准和因子核心链路现已统一到 DataHub read-only facade。审计发现并恢复了 3 个活跃行情文件中的旧测试污染行，同时补上行级完整性门禁。历史盘中监控仍留有多套行情 fallback，尚未完全实现“DataHub 是唯一数据入口”。当前最高风险收敛为遗留盘中数据所有权、监管公告真值缺失与真实券商验收。
...
- ST 名单已整改：交易侧只读 canonical `stock_basic`。监管名单改为只读 DataHub snapshot；当前 snapshot 缺失，因此 BUY pretrade 明确 `regulatory_truth_unavailable`，不再把网络失败当正常。
...
1. 将 `intraday_monitor.py`、`etf_dive_warning.py`、`monitor_588710.py` 和 dive predictor 的实时行情统一到 RealtimeQuoteEngine/QMT snapshot；禁止业务脚本自行 Eastmoney/Sina/Tencent/AkShare fallback。
2. 扩充 DataHub Event Truth ingestion，生成监管公告、forecast、holder trade、repurchase、share float、dividend；迁移 `semiconductor_events.py` 后删除业务层 provider 调用。
