...
- Strategy Lab 回测、SSE regime 与 universe 原来分别硬编码共享 K 线、项目根和旧 Baostock 行业副本。现回测/regime 通过 `daily_kline_path()` 读取并规范 canonical schema，主题池使用 canonical `stock_basic` 行业和版本控制标签，manual/today 输入统一引用 `SHARED_DATAHUB_ROOT`；专项 12 项通过。
