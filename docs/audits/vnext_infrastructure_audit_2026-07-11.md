...
- `etf_dive_warning.py`、`monitor_588710.py` 和 dive predictor 仍各自实现 AkShare/Eastmoney/Sina fallback。
...
1. 将 `etf_dive_warning.py`、`monitor_588710.py` 和 dive predictor 的实时行情统一到 DataHub canonical live snapshot/QMT snapshot；禁止业务脚本自行 Eastmoney/Sina/Tencent/AkShare fallback。
