...
export default function RiskDashboard() {
  ...
  const load = async () => {
    ...
    try {
      const [ov, al, ks, hi] = await Promise.all([
        ...
        fetch(`${API}/api/risk/alerts?limit=100`).then(r => r.json()),
        fetch(`${API}/api/risk/kill-switch`).then(r => r.json()),
        fetch(`${API}/api/risk/history?cycles=20&incidents_limit=50`).then(r => r.json()),
      ])
      setOverview(ov)
      setAlerts(al)
      setKillSwitch(ks)
  ...
  const alertCols = [
    ...
    },
    {
      ...
      render: (v) => {
        const colorMap = { open: 'error', acknowledged: 'processing', resolved: 'success', closed: 'default', resolving: 'warning' }
  ...
  const cycleCols = [
    ...
    { title: '规则数', dataIndex: 'n_rules', key: 'n_rules', width: 60, align: 'right' },
    { title: '违规', dataIndex: 'n_violations', key: 'n_violations', width: 60, align: 'right',
      render: v => v > 0 ? <span style={{ color: '#D97706', fontWeight: 600 }}>{v}</span> : v },
    { title: '阻塞', dataIndex: 'n_blockers', key: 'n_blockers', width: 60, align: 'right',
      render: v => v > 0 ? <span style={{ color: '#DC2626', fontWeight: 600 }}>{v}</span> : v },
    { title: '触发 KS', dataIndex: 'kill_switch_triggered', key: 'ks', width: 70, align: 'center',
      render: v => v ? <Tag color="error">是</Tag> : <Tag>否</Tag> },
    ...
    return (
      <div>
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card style={statCard(o.n_open_incidents > 0 ? '#D97706' : '#059669')} styles={{ body: { padding: '16px 20px' } }}>
              <Statistic
                title={<Text style={{ fontSize: 12, color: '#64748B', fontWeight: 500 }}>未关闭告警</Text>}
                value={o.n_open_incidents ?? 0}
                prefix={<BugOutlined />}
                valueStyle={{ color: o.n_open_incidents > 0 ? '#D97706' : '#059669', fontSize: 20, fontWeight: 700 }}
              />
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card style={statCard(o.n_blockers > 0 ? '#DC2626' : '#059669')} styles={{ body: { padding: '16px 20px' } }}>
              <Statistic
                title={<Text style={{ fontSize: 12, color: '#64748B', fontWeight: 500 }}>阻塞项</Text>}
                value={o.n_blockers ?? 0}
                prefix={<CloseCircleOutlined />}
                valueStyle={{ color: o.n_blockers > 0 ? '#DC2626' : '#059669', fontSize: 20, fontWeight: 700 }}
              />
            </Card>
          </Col>
        </Row>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: '时间', dataIndex: 'blocked_at', width: 170,
                      render: (v) => v ? <Text style={{ fontSize: 11, color: '#64748B' }}>{new Date(v).toLocaleString('zh-CN')}</Text> : '-',
                    },
            ...
            <Table
              ...
              columns={[
                ...
                { title: '解决时间', dataIndex: 'resolved_at', key: 'resolved_at', width: 170,
                  render: (v) => v ? <Text style={{ fontSize: 12, color: '#64748B' }}>{new Date(v).toLocaleString('zh-CN')}</Text> : '-',
                },
