...
export default function Feedback() {
  ...
  // ── Status update ──
  const updateItemStatus = async (fid, newStatus) => {
  ...
  const cols = [
    ...
    { title: '标题', dataIndex: 'title', key: 't', ellipsis: true,
      render: (v, r) => <a onClick={() => openDetail(r.id)} style={{ color: '#2563EB' }}>{v}</a>
    },
      ...
      {detailLoading
        ...
        : detail && <div>
            ...
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={6}>
                <Statistic title="分类" value={CATEGORY_CONFIG[detail.category]?.label || detail.category}
                  valueStyle={{ fontSize: 14, color: CATEGORY_CONFIG[detail.category]?.color }} />
              </Col>
              <Col span={6}>
                <Statistic title="状态"
                  value={<Tag color={STATUS_CONFIG[detail.status]?.color}>{STATUS_CONFIG[detail.status]?.label || detail.status}</Tag>}
            ...
            {/* Status Update */}
            <Divider orientation="left" style={{ fontSize: 12, color: '#64748B' }}>状态更新</Divider>
            ...
            <Divider orientation="left" style={{ fontSize: 12, color: '#64748B' }}>评论 ({detail.comments?.length || 0})</Divider>
