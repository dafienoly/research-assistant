...
interface DiagnosticsData {
  timestamp?: string
  hostname?: string
  system?: { platform?: string; python?: string }
  venv?: { exists?: boolean; path?: string }
  disk?: { status: string; usage_pct: number; total_gb: number; used_gb: number; free_gb: number }
  memory?: { status: string; usage_pct: number; total_gb: number; available_gb: number }
  python_deps?: Array<{ name: string; available: boolean }>
  git?: { has_changes: boolean; changed_files: number }
  workspace?: Record<string, boolean>
  cron?: { registered: boolean }
}

interface BackupResultData {
  success: boolean
  timestamp?: string
  results?: Record<string, { success: boolean; backup_id?: string; path?: string; error?: string }>
}
...
const RUNNING_CONFIG: Record<string, { color: string; icon: React.ReactNode; label: string; dot: string; bg: string; text: string }> = {
...
const STATUS_CONFIG: Record<string, { color: string; icon: React.ReactNode; label: string; dot: string }> = {
...
export default function OpsCenter() {
  ...
  const [backupResult, setBackupResult] = useState<BackupResultData | null>(null)
  const [logModal, setLogModal] = useState<{ visible: boolean; serviceId: string | null; logs: string[] }>({ visible: false, serviceId: null, logs: [] })
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [h, p] = await Promise.all([
        fetch(`${API}/api/ops/health`).then(r => r.json() as Promise<HealthData>),
        fetch(`${API}/api/ops/ports`).then(r => r.json() as Promise<{ ports?: PortEntry[] }>),
      ])
      setHealth(h)
      setPorts(p?.ports || [])
    } catch (e) {
      setError(e instanceof Error ? e.message : '加载运维中心失败')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const loadDiagnostics = async () => {
    setDiagLoading(true)
    try {
      const d = await fetch(`${API}/api/ops/diagnostics`).then(r => r.json() as Promise<DiagnosticsData>)
      setDiagnostics(d)
    } catch (e) {
      message.error('诊断加载失败: ' + (e instanceof Error ? e.message : String(e)))
    } finally {
  ...
  const doBackup = async () => {
    ...
    try {
      const r = await fetch(`${API}/api/ops/backup`, { method: 'POST' }).then(r => r.json() as Promise<BackupResultData>)
      setBackupResult(r)
      if (r.success) {
  ...
  const serviceAction = async (serviceId: string, action: string) => {
    ...
    try {
      const r = await fetch(`${API}/api/ops/${action}/${serviceId}`, { method: 'POST' }).then(r => r.json() as Promise<{ success: boolean; message?: string; error?: string }>)
      if (r.success) {
      ...
      message.error(`${action} ${serviceId} 失败: ${e instanceof Error ? e.message : String(e)}`)
    } finally {
      setActionLoading(prev => ({ ...prev, [key]: false }))
    }
  }

  const showLogs = (serviceId: string, logs: string[]) => {
  ...
  function renderServices() {
    ...
    const cols = [
      ...
      },
      {
        ...
        render: (v: number) => v ? <Text style={{ fontSize: 12 }}>{v}</Text> : '-',
      },
      {
        title: '检测来源', dataIndex: 'detected_by', key: 'detected_by', width: 80,
        render: (v: string) => {
          const map: Record<string, string> = { pid: 'PID', port: '端口', cron: 'Cron', none: '无' }
      ...
      },
      {
        ...
        render: (_: unknown, record: Record<string, unknown>) => {
          const sid = record.id as string
          const isRunning = record.running as boolean
          return (
            <Space size="small">
              <Button
                size="small"
              ...
              <Button
                ...
                onClick={() => showLogs(sid, (record.log_tail as string[]) || [])}
              >
                日志
              </Button>
            </Space>
  ...
  function renderPorts() {
    const portCols = [
      ...
      { title: '端口', dataIndex: 'port', key: 'port', width: 80,
        render: (v: number) => <code style={{ fontSize: 13 }}>{v}</code> },
  ...
  function renderDiagnostics() {
    const d = diagnostics || {} as DiagnosticsData
