...
import { Card, Row, Col, Table, Tag, Button, message, Modal, Descriptions, Progress, Typography, Tooltip, Alert } from 'antd'
import { ReloadOutlined, CheckCircleOutlined, CloseCircleOutlined, MinusCircleOutlined, RightCircleOutlined, PieChartOutlined } from '@ant-design/icons'
import { API } from '../App'
...
export default function Roadmap() {
  ...
  const [detail, setDetail] = useState<VersionItem | null>(null)
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const d = await r.json() as RoadmapResponse
      setData(d.versions || [])
      setProgress(d.progress || null)
    } catch (e) {
      setError(e instanceof Error ? e.message : '加载路线图失败')
    } finally {
      setLoading(false)
      setRefreshing(false)
    }
  ...
  const mark = async (version: string, status: string) => {
  ...
  if (loading && data.length === 0) return <LoadingState tip="加载路线图中..." />

  // ── Series progress ──
  const seriesList: SeriesProgress[] = progress?.series || []
  const bySeries: Record<string, VersionItem[]> = {}
  for (const v of (data || [])) {
  ...
  const cols = [
    {
      ...
      render: (v: string) => {
        ...
        return <span><StatusDot status={STATUS_DOT_MAP[v] || 'idle'} size={7} /> <Tag color={cfg.color} icon={cfg.icon}>{cfg.label}</Tag></span>
      },
    },
    { title: '版本', dataIndex: 'version', key: 'version', width: 80, render: (v: string) => <code style={{ color: '#2563EB', fontSize: 12 }}>{v}</code> },
    { title: '名称', dataIndex: 'name', key: 'name', width: 160 },
    { title: '目标', dataIndex: 'objective', key: 'objective', ellipsis: true },
    { title: '系列', dataIndex: 'series', key: 'series', width: 160, render: (v: string) => <Tag color="geekblue">{v?.replace(/^V\d\s/, '')}</Tag> },
    {
      title: '自动', dataIndex: 'auto_allowed', key: 'auto', width: 50,
      render: (v: boolean) => v ? <CheckCircleOutlined style={{ color: '#059669' }} /> : <CloseCircleOutlined style={{ color: '#94A3B8' }} />,
    },
    {
      title: '交易模式', dataIndex: 'trading_mode', key: 'trading_mode', width: 100,
      render: (v: string) => v && v !== 'none' ? <Tag color="purple">{v}</Tag> : <span style={{ color: '#94A3B8' }}>-</span>,
    },
    {
  ...
  const renderSummaryCards = () => {
    ...
    return <Row gutter={16} style={{ marginBottom: 8 }}>
      ...
      <Col span={4}>
        <Card style={{ ...statCardBase, borderLeft: p.failed && p.failed > 0 ? '4px solid #DC2626' : '4px solid #E2E8F0' }}>
          <Typography.Text style={{ color: '#64748B', fontSize: 12 }}>❌ 失败</Typography.Text>
          <div style={{ fontSize: 28, fontWeight: 700, color: p.failed && p.failed > 0 ? '#DC2626' : '#94A3B8' }}>{p.failed ?? '-'}</div>
        </Card>
      </Col>
      <Col span={4}>
  ...
  const renderSeriesProgress = () => {
    ...
    return <Row gutter={16} style={{ marginBottom: 12 }}>
      {seriesList.map((s: SeriesProgress) => {
  ...
  return <div className="stagger-fade">
    ...
    {error && data.length === 0 && <ErrorState message="加载失败" description={error} onRetry={() => load(true)} />}
    {error && data.length > 0 && <Alert message={error} type="error" showIcon style={{ marginBottom: 16, borderRadius: 8 }} closable />}
    {renderSummaryCards()}
    ...
    <Card title={<span style={{ color: '#0F172A', fontWeight: 600 }}>📋 版本清单</span>}
      extra={
        <span style={{ color: '#94A3B8', fontSize: 12 }}>
          当前版本: {progress?.current ? (data?.find((v: VersionItem) => v.status === 'current')?.version || '-') : '-'} •
          点击版本行查看详情 • 自动刷新 30s
        </span>
      }
      style={cardStyle}>
      ...
      <Table
        ...
        locale={{ emptyText: <EmptyState description="暂无路线图数据" /> }}
      />
    </Card>
        ...
        <Descriptions column={2} size="small" bordered
          ...
          <Descriptions.Item label="状态">
            {(() => {
              ...
              return <span><StatusDot status={STATUS_DOT_MAP[detail.status] || 'idle'} size={7} /> <Tag color={cfg.color} icon={cfg.icon}>{cfg.label}</Tag></span>
            })()}
          </Descriptions.Item>
