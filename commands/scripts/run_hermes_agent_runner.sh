...
{
  echo "[$(date)] === Hermes auto loop start ==="

  # 心跳
  $VENV -c "from factor_lab.leader.auto_loop import tick; tick()"

  # 自动执行：stdout/stderr 全部落盘，供 dashboard 实时展示
  $VENV $CLI leader:auto-run-once

  echo "[$(date)] auto loop done"
} >> /tmp/hermes_agent_runner.log 2>&1
