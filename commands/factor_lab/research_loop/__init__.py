...
class ResearchLoop:
    ...
    def _phase2_batch_backtest(self, candidates: list[dict]) -> list[dict]:
        """Phase 2: 并发回测（数据一次加载，多因子复用）"""
        print(f"  [Phase 2] 加载行情数据 + 并发回测 {len(candidates)} 个候选...")

        # 先加载数据一次
        shared_df = self._load_evaluation_data()
        if shared_df is None or len(shared_df) < 100:
            print("  ⚠️ 数据加载失败，所有候选标记为失败")
            for c in candidates:
                c.update({"score": 0, "ic_mean": 0, "status": "failed",
                          "error": "数据加载失败"})
            return candidates
        ...
        results = []
        # 限制并发数防止 OOM
        max_workers = min(self.config.max_concurrent, 3)
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futures = {
                pool.submit(self._evaluate_single, c["expression"], shared_df): c
                for c in candidates
            }
    ...
    def _evaluate_single(self, expression: str, df=None) -> dict:
        ...
        Args:
            expression: 因子表达式
            df: 可选，预加载的行情 DataFrame。为 None 时自动加载。
        """
        try:
            import warnings
            ...
            # 1. 获取行情数据（优先使用传入的 df，否则自动加载）
            if df is None:
                df = self._load_evaluation_data()
            if df is None or len(df) == 0:
...
{{"agreement": true|false, "review": "...", "suggested_decision": "adopt|reject|improve"}}"""
    ...
    def _phase4_update_notes(self, results: list[dict], best: Optional[dict]):
        ...
        if best and best.get("score", 0) > 0:
            self._enqueue_to_pipeline(best)

    def _enqueue_to_pipeline(self, candidate: dict):
        """将候选因子写入管线队列（事件触发）"""
        try:
            ...
            cfg = PipelineConfig()
            incoming = cfg.QUEUE_DIR / "incoming"
            incoming.mkdir(parents=True, exist_ok=True)

            name = candidate.get("name", f"candidate_{self.iteration}")
            task = {
                "name": name,
                "expression": candidate.get("expression", ""),
                "ic_mean": candidate.get("ic_mean", 0),
                "ic_ir": candidate.get("ic_ir", 0),
                "score": candidate.get("score", 0),
                "grade": candidate.get("grade", "D"),
                "source": f"research_loop_round_{self.iteration}",
                "created_at": datetime.now().isoformat(),
                "retry_count": 0,
                "status": "pending",
            }
            task_path = incoming / f"{name}.json"
            task_path.write_text(json.dumps(task, ensure_ascii=False, indent=2))
            print(f"  📥 已入队到自动管线: {name} (IC={task['ic_mean']:.4f})")
        except Exception as e:
            print(f"  ⚠️ 入队失败: {e}")
